...
document.getElementById('save-all').addEventListener('click', () => { saveToStorage(); applyToDOM(); closePanel(); showToast('Perubahan berhasil disimpan!'); });
...
function applyToDOM() {
    ...
    document.querySelector('.projects-container').innerHTML=data.projects.map(item=>{ const tags=item.tags.split(',').map(t=>'<span>'+t.trim()+'</span>').join(''); return '<div class="project-card reveal visible"><div class="project-img"><img src="image.png" alt="'+item.title+'"><div class="project-overlay"><a href="'+(item.demo||'#')+'" target="_blank" rel="noopener" class="btn">Live Demo</a><a href="'+(item.source||'#')+'" target="_blank" rel="noopener" class="btn btn-outline">Source Code</a></div></div><div class="project-info"><h3>'+item.title+'</h3><p>'+item.desc+'</p><div class="project-tags">'+tags+'</div></div></div>'; }).join('');
...
function handleCVFile(file){
    ...
    reader.onload=(e)=>{ const d=e.target.result,sz=(file.size/1024/1024).toFixed(2); localStorage.setItem('portfolioCVData',d); localStorage.setItem('portfolioCVName',file.name); localStorage.setItem('portfolioCVSize',sz+' MB'); localStorage.setItem('portfolioCVType',file.type); showCVStatus(file.name,d,sz+' MB',file.type); updateCVLinks(d); showToast('CV berhasil diupload!'); };
