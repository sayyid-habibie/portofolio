...
loginForm.addEventListener('submit', (e) => {
    ...
    if (doLogin(n, p)) {
        ...
        const ghToken = document.getElementById('login-gh-token').value.trim();
...
logoutBtn.addEventListener('click', () => { sessionStorage.removeItem('adm_auth'); sessionStorage.removeItem('gh_token'); closePanel(); showToast('Berhasil logout.'); });
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    fetch(apiUrl, { headers: { 'Authorization': 'Bearer ' + getToken(), 'Accept': 'application/vnd.github.v3+json' } })
    ...
    fetch(photoUrl, { headers: { 'Authorization': 'Bearer ' + getToken(), 'Accept': 'application/vnd.github.v3+json' } })
        .then(res => res.ok ? res.json() : null)
        .then(file => {
            if (file && file.content) {
                try {
...
async function syncPhotoToGitHub(key, url) {
    ...
    const headers = { 'Authorization': 'Bearer ' + getToken(), 'Accept': 'application/vnd.github.v3+json' };
